...
wlan_11n_deaggregate_pkt(mlan_private * priv, pmlan_buffer pmbuf)
{
    ...
    while (total_pkt_len > 0) {
        ...
        if ((pmadapter->callbacks.
             moal_malloc(pmadapter->pmoal_handle, sizeof(mlan_buffer),
                         MLAN_MEM_DEF, (t_u8 **) & daggr_mbuf))) {
            PRINTM(MERROR, "Error allocating daggr mlan_buffer\n");
            return MLAN_STATUS_FAILURE;
        }

        memcpy(pmadapter, daggr_mbuf, pmbuf, sizeof(mlan_buffer));
                       ...
                       int headroom, int ptrindex)
{
    ...
    if ((pmbuf_src =
        ...
        memcpy(pmadapter, pmbuf_aggr, pmbuf_src, sizeof(mlan_buffer));
