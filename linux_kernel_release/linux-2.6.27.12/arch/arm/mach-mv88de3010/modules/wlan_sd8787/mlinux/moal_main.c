...
#define REV_ID_REG      0x5c
        ...
        case SD8787_W1:
            ...
            break;
        case SD8787_A0:
            handle->drv_mode->fw_name = SD8787_A0_FW_NAME;
            break;
        default:
            break;
        }
...
#ifdef UAP_SUPPORT
    if ((priv->bss_type == MLAN_BSS_TYPE_UAP) && (priv->media_connected))
        carrier_on = MTRUE;
#endif
#ifdef STA_SUPPORT
...
woal_reassociation_thread(void *data)
{
    ...
    for (;;) {
        ...
        for (i = 0; i < handle->priv_num && (priv = handle->priv[i]); i++) {
            ...
            status = woal_set_ewpa_mode(priv,
                                        MOAL_CMD_WAIT, &priv->prev_ssid_bssid);
