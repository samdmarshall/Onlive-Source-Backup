	...
	)
{
	...
	// disable INTR
	//galois_interrupt_mask(GFXHW_cpu_id, GFXHW_intr_vec);
	PIC_SetPerPIC_PerVector(GFXHW_cpu_id, GFXHW_intr_vec, 0, 0);
	...
	galois_printk("Clear GiQuila INTR = %08x\n", data);
	// Enable INTR
	PIC_SetPerPIC_PerVector(GFXHW_cpu_id, GFXHW_intr_vec, 1, 1);
	//galois_interrupt_unmask(GFXHW_cpu_id, GFXHW_intr_vec);
    ...
    )
{
    ...
    if (physical != 0)
    {
	    ...
	    {
    	    gcmTRACE_ZONE("[galcore] gcoGALDEVICE_Construct: Unable to map location->0x%lX for size->%ld\n",
			  RegisterMemBase,
			  RegisterMemSize);
