...
wlan_get_cfp_table_ioctl(wlan_private * priv, struct iwreq *wrq)
{
    ...
    cfp = wlan_get_region_cfp_table((u8) regioncode, BAND_G | BAND_B, &cfp_no);
...
wlan_do_ioctl(struct net_device *dev, struct ifreq *req, int cmd)
{
    ...
    case WLAN_SET_GET_256_CHAR:
        switch (wrq->u.data.flags) {
        case WLANPASSPHRASE:
            ret = wlan_setpassphrase(priv, wrq);
            break;
        default:
            ret = -EOPNOTSUPP;
            break;
...
#define MAX_U16_VAL	65535
    ...
    case WLAN_SETNONE_GETONEINT:
        ...
        case WLAN_GET_TX_RATE:
            ...
            break;
        case WLAN_GETRSNMODE:
            ret = wlan_get_rsnmode_ioctl(priv, req);
            break;
        case WLAN_GETACTCIPHER:
            ret = wlan_get_currentpairwisecipher_ioctl(priv, req);
            break;
        case WLAN_GETACTGROUPCIPHER:
            ret = wlan_get_currentgroupcipher_ioctl(priv, req);
            break;
        case WLANGETDTIM:
