               ...
               struct iw_point *dwrq, char *extra)
{
    ...
    if (Adapter->InfrastructureMode == Wlan802_11Infrastructure) {
        ...
        if ((dwrq->flags & IW_ENCODE_INDEX) > 1) {
            ...
            Adapter->SecInfo.EWPAEnabled = isSTASecEnabled(priv, reqSSID,
                                                           Adapter->
                                                           ScanTable[i].
                                                           MacAddress);
            i = wlan_find_ssid_in_list(Adapter, &reqSSID, NULL,
    ...
    if (!memcmp(bcast, awrq->sa_data, ETH_ALEN))
...

        {
            i = wlan_find_bssid_in_list(Adapter, reqBSSID,
                                        Wlan802_11AutoUnknown);
            Adapter->SecInfo.EWPAEnabled =
                isSTASecEnabled(priv, Adapter->ScanTable[i].Ssid, reqBSSID);
        }
...
wlan_associate_to_table_idx(wlan_private * priv, int tableIdx)
{
    ...
    Adapter->SecInfo.EWPAEnabled =
        isSTASecEnabled(priv, Adapter->ScanTable[tableIdx - 1].Ssid,
                        Adapter->ScanTable[tableIdx - 1].MacAddress);
    /* 
    ...
    if (Adapter->MediaConnectStatus == WlanMediaStateDisconnected) {
        if (Adapter->InfrastructureMode == Wlan802_11Infrastructure) {
            if (memcmp(Adapter->PreviousBSSID, zeroMac, sizeof(zeroMac)) != 0) {
                ...
                Adapter->SecInfo.EWPAEnabled =
                    isSTASecEnabled(priv, Adapter->PreviousSSID,
                                    Adapter->PreviousBSSID);
                i = wlan_find_ssid_in_list(Adapter, &Adapter->PreviousSSID,
                                           Adapter->PreviousBSSID,
                                           Adapter->InfrastructureMode);
                          ...
                          HostCmd_DS_COMMAND * cmd, void *pdata_buf)
{
    ...
    if (!Adapter->wps.SessionEnable) {
        if ((Adapter->SecInfo.WPAEnabled || Adapter->SecInfo.WPA2Enabled)) {
        ...
        } else if (Adapter->SecInfo.EWPAEnabled) {
            pRsnTlv = (MrvlIEtypes_RsnParamSet_t *) pos;
            if (pBSSDesc->pWpaIE) {
                pRsnTlv->Header.Type =
                    (u16) (*(pBSSDesc->pWpaIE)).VendHdr.ElementId;
                pRsnTlv->Header.Type = pRsnTlv->Header.Type & 0x00FF;
                pRsnTlv->Header.Type = wlan_cpu_to_le16(pRsnTlv->Header.Type);
                pRsnTlv->Header.Len = (u16) (*(pBSSDesc->pWpaIE)).VendHdr.Len;
                pRsnTlv->Header.Len = pRsnTlv->Header.Len & 0x00FF;
                if (pRsnTlv->Header.Len <= (sizeof(Adapter->Wpa_ie))) {
                    memcpy(pRsnTlv->RsnIE,
                           &((*(pBSSDesc->pWpaIE)).VendHdr.Oui[0]),
                           pRsnTlv->Header.Len);
                } else {
                    ret = WLAN_STATUS_FAILURE;
                    goto done;
            ...
            if (pBSSDesc->pRsnIE) {
                pRsnTlv = (MrvlIEtypes_RsnParamSet_t *) pos;
                pRsnTlv->Header.Type =
                    (u16) (*(pBSSDesc->pRsnIE)).IeeeHdr.ElementId;
                pRsnTlv->Header.Type = pRsnTlv->Header.Type & 0x00FF;
                pRsnTlv->Header.Type = wlan_cpu_to_le16(pRsnTlv->Header.Type);
                pRsnTlv->Header.Len = (u16) (*(pBSSDesc->pRsnIE)).IeeeHdr.Len;
                pRsnTlv->Header.Len = pRsnTlv->Header.Len & 0x00FF;
                if (pRsnTlv->Header.Len <= (sizeof(Adapter->Wpa_ie) - 2)) {
                    memcpy(pRsnTlv->RsnIE, &((*(pBSSDesc->pRsnIE)).Data[0])
                           , pRsnTlv->Header.Len);
                } else {
                    ret = WLAN_STATUS_FAILURE;
                    goto done;
