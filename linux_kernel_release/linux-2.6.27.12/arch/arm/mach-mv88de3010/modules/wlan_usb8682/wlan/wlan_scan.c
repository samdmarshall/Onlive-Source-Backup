...
wlan_is_network_compatible(wlan_adapter * Adapter, int index, int mode)
{
    ...
    if (Adapter->SecInfo.EWPAEnabled) {
        if (((pBSSDesc->pWpaIE) &&
            ...
            PRINTM(INFO, "EWPAEnabled: Ignore none WPA/WPA2 AP\n");
            LEAVE();
            return -ECONNREFUSED;
        }
    }
              ...
              struct iw_point *dwrq, char *extra)
{
    ...
    for (i = 0; i < Adapter->NumInScanTable; i++) {
        ...
        cfp =
            find_cfp_by_band_and_channel(Adapter, 0, (u16) pScanTable->Channel);
        if (!cfp) {
        ...
        for (j = 0; j < sizeof(Adapter->ScanTable[i].SupportedRates); j++) {
            ...
            current_val = iwe_stream_add_value(info, current_ev, current_val, end_buf,
                                               &iwe, iwe.len);
