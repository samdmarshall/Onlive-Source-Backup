...
wlan_dnld_cmd_to_fw(wlan_private * priv, CmdCtrlNode * CmdNode)
{
    ...
    ret = sbi_host_to_card(priv, MV_TYPE_CMD, (u8 *) CmdPtr, CmdSize, NULL);
                 ...
                 u16 wait_option, WLAN_OID cmd_oid, void *pdata_buf)
{
    ...
    case HostCmd_CMD_SUPPLICANT_PMK:
        ret = wlan_cmd_802_11_supplicant_pmk(priv, CmdPtr,
                                             cmd_action, pdata_buf);
        break;
    case HostCmd_CMD_SUPPLICANT_PROFILE:
        ret = wlan_cmd_802_11_supplicant_profile(priv, CmdPtr,
                                                 cmd_action, pdata_buf);
        break;
...
wlan_alloc_cmd_buffer(wlan_private * priv)
{
    ...
    if (!(TempCmdArray = kmalloc(ulBufSize, GFP_KERNEL))) {
            PRINTM(ERROR, "ALLOC_CMD_BUF: Failed to allocate %d TempCmdArray\n", ulBufSize);
        ret = WLAN_STATUS_FAILURE;
        goto done;
    }
    ...
    for (i = 0; i < MRVDRV_NUM_OF_CMD_BUFFER; i++) {
        if (!(pTempVirtualAddr = kmalloc(ulBufSize, GFP_KERNEL))) {
                PRINTM(ERROR, "ALLOC_CMD_BUF: pTempVirtualAddr: %d(@%p) out of memory\n", ulBufSize, pTempVirtualAddr );
            ret = WLAN_STATUS_FAILURE;
            goto done;
        }
...
wlan_dnld_sleep_confirm_cmd(wlan_private * priv, u8 * CmdPtr, u16 size)
{
    ...
    ret = sbi_host_to_card(priv, MV_TYPE_CMD, CmdPtr, size, NULL);
