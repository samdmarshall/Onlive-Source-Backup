             ...
             struct sockaddr *awrq, char *extra)
{
    ...
    if (MLAN_STATUS_SUCCESS !=
        woal_associate(priv, MOAL_IOCTL_WAIT, &ssid_bssid)) {
        ret = -EFAULT;
        goto done;
    }
#ifdef REASSOCIATION
               ...
               struct iw_param *vwrq, char *extra)
{
    ...
    PRINTM(INFO, "PS_MODE set power disabled=%d\n", vwrq->disabled);
    if (vwrq->disabled)
        pm_cfg->param.ps_mode = 0;
    else {
        /* Check not support case only (vwrq->disabled == FALSE) */
        if ((vwrq->flags & IW_POWER_TYPE) == IW_POWER_TIMEOUT) {
            PRINTM(ERROR, "Setting power timeout command is not supported\n");
            ret = -EINVAL;
            goto done;
        } else if ((vwrq->flags & IW_POWER_TYPE) == IW_POWER_PERIOD) {
            PRINTM(ERROR, "Setting power period command is not supported\n");
            ret = -EINVAL;
            goto done;
        }
                ...
                struct iw_point *dwrq, char *extra)
{
    ...
    if (index > 3) {
        PRINTM(ERROR, "Key index #%d out of range\n", index);
        ret = -EINVAL;
        goto done;
    }
    ...
    if (dwrq->length) {
        if (dwrq->length > MAX_WEP_KEY_SIZE) {
            PRINTM(ERROR, "Key length (%d) out of range\n", dwrq->length);
            ret = -EINVAL;
            goto done;
        }
        if (index < 0)
        ...
        if (dwrq->flags & IW_ENCODE_DISABLED) {
            PRINTM(INFO, "*** iwconfig mlanX key off ***\n");
             ...
             */
            if (index < 0) {
                PRINTM(INFO, "*** iwconfig mlanX key on ***\n");
                sec->param.encrypt_key.is_current_wep_key = MTRUE;
            } else
                sec->param.encrypt_key.key_index = index;
        }
              ...
              struct iw_request_info *info, struct iw_point *dwrq, char *extra)
{
    ...
    if ((mlme->cmd == IW_MLME_DEAUTH) || (mlme->cmd == IW_MLME_DISASSOC)) {
        if (MLAN_STATUS_SUCCESS !=
            woal_disconnect(priv, (t_u8 *) mlme->addr.sa_data))
            ret = -EFAULT;
    }
    LEAVE();
    return ret;
              ...
              struct iw_param *vwrq, char *extra)
{
    ...
    if (MOAL_ACQ_SEMAPHORE_BLOCK(&handle->reassoc_sem)) {
        PRINTM(ERROR, "Acquire semaphore error, woal_set_essid\n");
        LEAVE();
        return -EBUSY;
    }
#endif /* REASSOCIATION */
               ...
               struct iw_point *dwrq, char *extra)
{
    ...
    if (!dwrq->flags) {
        ...
        if (!req_ssid.ssid_len || req_ssid.ssid[0] < 0x20) {
            PRINTM(ERROR, "Invalid SSID - aborting set_essid\n");
            ret = -EINVAL;
            goto setessid_ret;
        }
              ...
              struct iw_point *dwrq, char *extra)
{
    ...
    for (i = 0; i < scan_resp.num_in_scan_table; i++) {
        ...
        if (!scan_table[i].freq) {
            PRINTM(ERROR, "Invalid channel number %d\n",
