    ...
    if ((pbss_desc->bss_mode == mode) &&
        ...
        if (((pbss_desc->pwpa_ie) &&
            ...
            if (((pmpriv->adapter->config_bands & BAND_GN) &&
                ...
                PRINTM(INFO, "AES not supported by AP when 11n enabled\n");
                LEAVE();
                return -1;
            }
            LEAVE();
            ...
            PRINTM(INFO, "ewpa_enabled: Ignore none WPA/WPA2 AP\n");
            LEAVE();
            return -1;
        }
    }
                            ...
                            OUT t_u8 * pscan_current_only)
{
    ...
    *pscan_current_only = MFALSE;
    ...
    if (puser_scan_in) {
        ...
        pscan_cfg_out->bss_type =
            (puser_scan_in->bss_type ? puser_scan_in->bss_type : pmadapter->
             scan_mode);
        ...
        for (ssid_idx = 0; ((ssid_idx < NELEMENTS(puser_scan_in->ssid_list))
            ...
            PRINTM(INFO, "Scan: ssid_list[%d]: %s, %d\n",
                   ssid_idx,
        ...
        pscan_cfg_out->bss_type = pmadapter->scan_mode;
    ...
    PRINTM(INFO, "SCAN_CMD: Rates size = %d\n", rates_size);
                                  ...
                                  OUT MrvlIEtypes_Data_t ** pptlv)
{
    ...
    *pptlv = MNULL;
    ...
    PRINTM(INFO, "SCAN_RESP: tlv_buf_size = %d\n", tlv_buf_size);
        ...
        if (req_tlv_type == tlv_type) {
            ...
            case TLV_TYPE_CHANNELBANDLIST:
                ...
                *pptlv = (MrvlIEtypes_Data_t *) pcurrent_tlv;
                break;
            default:
                PRINTM(ERROR, "SCAN_RESP: Unhandled TLV = %d\n", tlv_type);
                                ...
                                IN t_u8 ** pbeacon_info, IN t_u32 * bytes_left)
{
    ...
    *bytes_left -= beacon_size;
    ...
    if (bytes_left_for_current_beacon < 12) {
        PRINTM(ERROR, "InterpretIE: Not enough bytes left\n");
        LEAVE();
        return MLAN_STATUS_FAILURE;
    }
    ...
    PRINTM(INFO, "InterpretIE: fixed_ie.capabilities=0x%X\n",
    ...
    while (bytes_left_for_current_beacon >= 2) {
        ...
        if (bytes_left_for_current_beacon < total_ie_len) {
            PRINTM(ERROR, "InterpretIE: Error in processing IE, "
        ...
        case VENDOR_SPECIFIC_221:
            ...
            if (!memcmp(pvendor_ie->vend_hdr.oui, wpa_oui, sizeof(wpa_oui))) {
                ...
                pbss_entry->pwpa_ie =
                    (IEEEtypes_VendorSpecific_t *) (pbss_entry->pbeacon_buf +
        ...
        case HT_OPERATION:
            ...
            pbss_entry->pht_info =
                (IEEEtypes_HTInfo_t *) (pbss_entry->pbeacon_buf +
        ...
        case BSSCO_2040:
            ...
            pbss_entry->pbss_co_2040 =
                (IEEEtypes_2040BSSCo_t *) (pbss_entry->pbeacon_buf +
                                           pbss_entry->bss_co_2040_offset);
        ...
        case EXT_CAPABILITY:
            ...
            pbss_entry->pext_cap =
                (IEEEtypes_ExtCap_t *) (pbss_entry->pbeacon_buf +
...
wlan_scan_delete_table_entry(IN mlan_private * pmpriv, IN t_s32 table_idx)
{
    ...
    PRINTM(INFO, "Scan: Delete Entry %d, num_in_scan_table = %d\n",
                   ...
                   IN const wlan_user_scan_cfg * puser_scan_in)
{
    ...
    if (ret != MLAN_STATUS_SUCCESS || !pscan_cfg_out) {
        PRINTM(ERROR, "Memory allocation for pscan_cfg_out failed!\n");
        LEAVE();
        return MLAN_STATUS_FAILURE;
    }
    ...
    if (ret != MLAN_STATUS_SUCCESS || !pscan_chan_list) {
        PRINTM(ERROR, "Failed to allocate scan_chan_list\n");
        if (pscan_cfg_out)
                     ...
                     IN HostCmd_DS_COMMAND * pcmd, IN t_void * pdata_buf)
{
    ...
    pscan_cmd->bss_type = pscan_cfg->bss_type;
    ...
    pcmd->size = wlan_cpu_to_le16(sizeof(pscan_cmd->bss_type)
                                  + sizeof(pscan_cmd->bssid)
                     ...
                     IN HostCmd_DS_COMMAND * resp, IN t_void * pioctl_buf)
{
    ...
    if (pscan_rsp->number_of_sets > MRVDRV_MAX_BSSID_LIST) {
        PRINTM(ERROR, "SCAN_RESP: Invalid number of AP returned (%d)!!\n",
    ...
    PRINTM(INFO, "SCAN_RESP: bss_descript_size %d\n", bytes_left);

    scan_resp_size = resp->size;

    PRINTM(INFO, "SCAN_RESP: returned %d APs before parsing\n",
           pscan_rsp->number_of_sets);
    ...
    if (ret != MLAN_STATUS_SUCCESS || !bss_new_entry) {
        PRINTM(ERROR, "Memory allocation for bss_new_entry failed!\n");
        ret = MLAN_STATUS_FAILURE;
        goto done;
    }
    ...
    if (!util_peek_list
        ...
        if (pioctl_req != MNULL) {
            ...
            pcb->moal_ioctl_complete(pmadapter->pmoal_handle,
                                     pioctl_buf, MLAN_STATUS_SUCCESS);
        }
    } else {
        /* Get scan command from scan_pending_q and put to cmd_pending_q */
        pcmd_node =
                       ...
                       OUT mlan_ssid_bssid * preq_ssid_bssid)
{
    ...
    PRINTM(INFO, "Best network found = [%s], "
                        ...
                        IN t_void * pioctl_buf, IN mlan_802_11_ssid * preq_ssid)
{
    ...
    if (ret != MLAN_STATUS_SUCCESS || !pscan_cfg) {
        PRINTM(ERROR, "Memory allocation for pscan_cfg failed!\n");
        ret = MLAN_STATUS_FAILURE;
        goto done;
    }
    ...
    if ((pmpriv->pcurr_bcn_buf == MNULL) ||
        ...
        if ((ret == MLAN_STATUS_SUCCESS) && pmpriv->pcurr_bcn_buf) {
            memcpy(pmpriv->pcurr_bcn_buf, pcurr_bss->pbeacon_buf,
                   pcurr_bss->beacon_buf_size);
            PRINTM(INFO, "current beacon saved %d\n", pmpriv->curr_bcn_size);
        }
    }
}
...
wlan_restore_curr_bcn(IN mlan_private * pmpriv)
{
    ...
    if (pmpriv->pcurr_bcn_buf &&
        ...
        PRINTM(INFO, "current beacon restored %d\n", pmpriv->curr_bcn_size);
    } else {
        PRINTM(WARN, "curr_bcn_buf not saved or bcn_buf has no space\n");
    }
}
