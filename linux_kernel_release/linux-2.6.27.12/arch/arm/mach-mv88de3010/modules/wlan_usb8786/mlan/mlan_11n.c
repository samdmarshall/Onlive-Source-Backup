                               ...
                               IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (!wlan_is_cmd_allowed(pmpriv)) {
        PRINTM(MSG, "11N_CFG: not allowed\n");
        ret = MLAN_STATUS_FAILURE;
        goto exit;
    }

    PRINTM(ERROR, "Prepare the command\n");
    cfg = (mlan_ds_11n_cfg *) pioctl_req->pbuf;
    if (pioctl_req->action == MLAN_ACT_SET)
                            ...
                            IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (pioctl_req->action == MLAN_ACT_GET) {
        PRINTM(ERROR, "Addba reject MLAN\n");
        memcpy(cfg->param.addba_reject, pmpriv->addba_reject, MAX_NUM_TID);
    } else {
        if (pmpriv->media_connected == MTRUE) {
            PRINTM(ERROR, "Can not set aggr priority table in connected"
                   " state\n");
            LEAVE();
...
wlan_show_dot11ndevcap(pmlan_adapter pmadapter, t_u32 cap)
{
    ...
    PRINTM(INFO, "GET_HW_SPEC: Maximum MSDU length = %s octets\n",
           (ISSUPP_MAXAMSDU(cap) ? "7935" : "3839"));
    PRINTM(INFO, "GET_HW_SPEC: Beam forming %s\n",
           (ISSUPP_BEAMFORMING(cap) ? "supported" : "not supported"));
    PRINTM(INFO, "GET_HW_SPEC: Greenfield preamble %s\n",
           (ISSUPP_GREENFIELD(cap) ? "supported" : "not supported"));
    PRINTM(INFO, "GET_HW_SPEC: AMPDU %s\n",
           (ISSUPP_AMPDU(cap) ? "supported" : "not supported"));
    PRINTM(INFO, "GET_HW_SPEC: MIMO Power Save %s\n",
           (ISSUPP_MIMOPS(cap) ? "supported" : "not supported"));
    PRINTM(INFO, "GET_HW_SPEC: Rx STBC %s\n",
           (ISSUPP_RXSTBC(cap) ? "supported" : "not supported"));
    PRINTM(INFO, "GET_HW_SPEC: Tx STBC %s\n",
           (ISSUPP_TXSTBC(cap) ? "supported" : "not supported"));
    PRINTM(INFO, "GET_HW_SPEC: Short GI for 40 Mhz %s\n",
           (ISSUPP_SHORTGI40(cap) ? "supported" : "not supported"));
    PRINTM(INFO, "GET_HW_SPEC: Short GI for 20 Mhz %s\n",
           (ISSUPP_SHORTGI20(cap) ? "supported" : "not supported"));
    PRINTM(INFO, "GET_HW_SPEC: LDPC coded packet receive %s\n",
           (ISSUPP_RXLDPC(cap) ? "supported" : "not supported"));
    PRINTM(INFO, "GET_HW_SPEC: Number of Delayed Block Ack streams = %d\n",
           GET_DELAYEDBACK(cap));
    PRINTM(INFO, "GET_HW_SPEC: Number of Immediate Block Ack streams = %d\n",
           GET_IMMEDIATEBACK(cap));
    PRINTM(INFO, "GET_HW_SPEC: 40 Mhz channel width %s\n",
           (ISSUPP_CHANWIDTH40(cap) ? "supported" : "not supported"));
    PRINTM(INFO, "GET_HW_SPEC: 20 Mhz channel width %s\n",
           (ISSUPP_CHANWIDTH20(cap) ? "supported" : "not supported"));
    PRINTM(INFO, "GET_HW_SPEC: 10 Mhz channel width %s\n",
           (ISSUPP_CHANWIDTH10(cap) ? "supported" : "not supported"));

    if (ISSUPP_RXANTENNAA(cap)) {
        PRINTM(INFO, "GET_HW_SPEC: Prescence of Rx antennea A\n");
    }
    if (ISSUPP_RXANTENNAB(cap)) {
        PRINTM(INFO, "GET_HW_SPEC: Prescence of Rx antennea B\n");
    }
    if (ISSUPP_RXANTENNAC(cap)) {
        PRINTM(INFO, "GET_HW_SPEC: Prescence of Rx antennea C\n");
    }
    if (ISSUPP_RXANTENNAD(cap)) {
        PRINTM(INFO, "GET_HW_SPEC: Prescence of Rx antennea D\n");
    }
    if (ISSUPP_TXANTENNAA(cap)) {
        PRINTM(INFO, "GET_HW_SPEC: Prescence of Tx antennea A\n");
    }
    if (ISSUPP_TXANTENNAB(cap)) {
        PRINTM(INFO, "GET_HW_SPEC: Prescence of Tx antennea B\n");
    }
    if (ISSUPP_TXANTENNAC(cap)) {
        PRINTM(INFO, "GET_HW_SPEC: Prescence of Tx antennea C\n");
    }
    if (ISSUPP_TXANTENNAD(cap)) {
        PRINTM(INFO, "GET_HW_SPEC: Prescence of Tx antennea D\n");
    }
...
wlan_show_devmcssupport(pmlan_adapter pmadapter, t_u8 support)
{
    ...
    PRINTM(INFO, "GET_HW_SPEC: MCSs for %dx%d MIMO\n", GET_RXMCSSUPP(support),
...
wlan_ret_11n_addba_req(mlan_private * priv, HostCmd_DS_COMMAND * resp)
{
    ...
    if (padd_ba_rsp->status_code == BA_RESULT_SUCCESS) {
        if ((ptx_ba_tbl = wlan_11n_get_txbastream_tbl(priv, tid,
                                                      padd_ba_rsp->
                                                      peer_mac_addr))) {
            PRINTM(INFO, "BA stream complete\n");
            ptx_ba_tbl->ba_status = BA_STREAM_SETUP_COMPLETE;
        } else {
            PRINTM(ERROR, "Stream not created!!!\n");
        }
    } else {
                        ...
                        IN BSSDescriptor_t * pbss_desc, OUT t_u8 ** ppbuffer)
{
    ...
    if (pbss_desc->pht_cap) {
        ...
        pht_cap->header.type = wlan_cpu_to_le16(TLV_TYPE_HT_CAP);
    ...
    if (pbss_desc->pbss_co_2040) {
        ...
        p2040_bss_co->header.type =
            wlan_cpu_to_le16(TLV_TYPE_2040BSS_COEXISTENCE);
    ...
    if (pbss_desc->pext_cap) {
        ...
        pext_cap->header.type = wlan_cpu_to_le16(TLV_TYPE_EXTCAP);
...
wlan_cfg_tx_buf(mlan_private * pmpriv, BSSDescriptor_t * pbss_desc)
{
    ...
    PRINTM(INFO, "max_amsdu=%d, maxTxBuf=%d\n", max_amsdu,
...
wlan_11n_cfg_ioctl(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (pioctl_req->buf_len < sizeof(mlan_ds_11n_cfg)) {
        PRINTM(INFO, "MLAN bss IOCTL length is too short.\n");
                                     ...
                                     TxBAStreamTbl * ptx_tbl)
{
    ...
    PRINTM(INFO, "tx_ba_stream_tbl_ptr %p\n", ptx_tbl);
...
wlan_11n_get_txbastream_tbl(mlan_private * priv, int tid, t_u8 * ra)
{
    ...
    while (ptx_tbl != (TxBAStreamTbl *) & priv->tx_ba_stream_tbl_ptr) {
        ...
        PRINTM(DAT_D, "get_txbastream_tbl TID %d", ptx_tbl->tid);
        DBG_HEXDUMP(DAT_D, "RA", ptx_tbl->ra, MLAN_MAC_ADDR_LENGTH);
                               ...
                               t_u8 * ra, int tid, baStatus_e ba_status)
{
    ...
    if (!wlan_11n_get_txbastream_tbl(priv, tid, ra)) {
        PRINTM(DAT_D, "get_txbastream_tbl TID %d", tid);
        DBG_HEXDUMP(DAT_D, "RA", ra, MLAN_MAC_ADDR_LENGTH);
...
wlan_send_addba(mlan_private * priv, int tid, t_u8 * peer_mac)
{
    ...
    PRINTM(CMND, "Send addba: TID %d\n", tid);
    DBG_HEXDUMP(CMD_D, "Send addba RA", peer_mac, MLAN_MAC_ADDR_LENGTH);
...
wlan_11n_delete_bastream(mlan_private * priv, t_u8 * del_ba)
{
    ...
    DBG_HEXDUMP(CMD_D, "Delba:", (t_u8 *) pdel_ba, 20);
    pdel_ba->del_ba_param_set = wlan_le16_to_cpu(pdel_ba->del_ba_param_set);
    pdel_ba->reason_code = wlan_le16_to_cpu(pdel_ba->reason_code);
