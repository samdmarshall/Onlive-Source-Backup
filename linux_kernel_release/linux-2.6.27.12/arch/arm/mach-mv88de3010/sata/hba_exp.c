...
void HBA_HandleWaitingList(PHBA_Extension pHBA)
{
	...
	while ( !List_Empty(&pHBA->Waiting_Request) ) {
		...
		if ( HBA_CanHandleRequest(pReq) ) {
			...
			//TBD: performance
			//Send request to pCore_Driver_Extension.
			HBA_GetNextModuleSendFunction(pHBA, &pNextExtension, &pNextFunction);
...
void HBA_ModuleStarted(MV_PVOID extension)
{
	...
	if ( module_id==0 ) /* HBA module */
	{
		MV_ERROR_PRINT("success to init chip(SATA)\n");
	...
	)
{
	...
	pController->Base_Address = pHBA->Base_Address;
...
MV_VOID HBA_GetNextModuleSendFunction(
...
MV_PVOID HBA_GetModuleExtension(
	IN MV_PVOID self_extension,
	IN MV_U8 module_id
	)
{
