...
/*
 * NOTE: pReq->SG_Table->Entry_Ptr is init in HBA_ModuleInitialize
 */
void SGTable_Append(
	OUT PMV_SG_Table pSGTable,
	MV_U32 address,
...
//	PMV_SG_Entry pSGEntry = &pSGTable->Entry[pSGTable->Valid_Entry_Count];
	 ...
	 */
	if ( size%2 ) {
		printk(KERN_ERR "[Berlin]PRD_SIZE_WORD_ALIGN not aligned: size=0x%08x!!!\n", size);
...
{
	MV_PRINT("Command header structure:\n");
	MV_PRINT("\tFIS_Length=0x%02x, ATAPI=%d, RESET=%d, NCQ=%d, PMP=%d, PRDTL=%d\n", 
			header->FIS_Length, header->Packet_Command, 
			header->Reset, header->NCQ, header->PM_Port, header->PRD_Entry_Count);
	MV_PRINT("\tTable_Address=0x%08x, Table_Address_High=0x%08x\n", 
			header->Table_Address, header->Table_Address_High);
}

void MV_DumpFIS(PSATA_FIS_REG_H2D pFIS)
{
	MV_PRINT("CFIS structure:\n");
	MV_PRINT("\tFIS_Type=0x%02x, C=%d, PMP=%d\n", pFIS->FIS_Type, pFIS->C, pFIS->PM_Port);
	MV_PRINT("\tCommand=0x%02x, Control=0x%02x, Device=0x%02x\n", 
			pFIS->Command, pFIS->Control, pFIS->Device);
	MV_PRINT("\tFeatures=0x%02x, Features_exp=0x%02x\n", pFIS->Features, pFIS->Features_Exp);
	MV_PRINT("\tLBA_low=0x%02x, LBA_mid=0x%02x, LBA_high=0x%02x\n", 
			pFIS->LBA_Low, pFIS->LBA_Mid, pFIS->LBA_High);
	MV_PRINT("\tLBA_low_exp=0x%02x, LBA_mid_exp=0x%02x, LBA_high_exp=0x%02x\n", 
			pFIS->LBA_Low_Exp, pFIS->LBA_Mid_Exp, pFIS->LBA_High_Exp);
	MV_PRINT("\tSector_count=0x%02x, Sector_count_exp=0x%02x\n", 
...
//		pSGEntry = &pSGTable->Entry[i];
		...
		MV_PRINT("\t#%d entry(PRD): addr(0x%08x%08x), size(0x%x).\n", 
