...
void __init pci_common_init(struct hw_pci *hw)
{
	...
	list_for_each_entry(sys, &hw->buses, node) {
		...
		if (!use_firmware) {
			...
			/*
			 * Enable bridges.
			 * For now, I enable all bridges 
			 * connected to the root complex, 
			 * It can be moved to PCIe host 
			 * fixup routine or specific bridge 
			 * fixup routine. -Qingchao
			 */
...
#ifndef CONFIG_MV88DE3010_PCIE
		phys = vma->vm_pgoff + (root->mem_offset >> PAGE_SHIFT);
#else
		/* Qingchao:
		 * root->mem_offset is used to spcify the offset between downstream
		 * bus-centric address and linux-centric address, actually
		 * vma->vm_pgoff is the real resource phys of each devices for mmap 
		 * and unmmap, and we do not need to care about root->mem_offset here. 
		 */
		phys = vma->vm_pgoff;
#endif
	}
