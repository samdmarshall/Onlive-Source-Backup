...
*   Modified by OnLive, Inc. Nov 2010.
 *
...
static int __init check_hex( char ch )
{
	if( (ch >= '0' && ch <= '9') ||
	    (ch >= 'a' && ch <= 'f') ||
	    (ch >= 'A' && ch <= 'F')
...
static void __init create_command_line(char *boot_command_line, char *from, unsigned int sz)
{
	char *fixed        = "mtdparts=mv_nand:64M(bootloader),72M(ra),72M(rb),15M(conf),-(r)";
	char *macaddr      = "macaddr=";
	int   len          = 0;

	*boot_command_line = 0;

	/// first copy the hardcoded command line
	len = strlen( fixed );

	if( len < sz )
	{
		memcpy( boot_command_line, fixed, len );
	...
	/// next, copy macaddr=, if we can find it...
	len = strlen( macaddr );
	while( memcmp( from, macaddr, len ) )
	{
		for( ; *from != ' ' ; from++ )
		{
			if( *from == 0 )
	...
	if( check_hex( from[len+ 0] ) && check_hex( from[len+ 1] ) &&
	    from[len+ 2] == ':'       &&
	    check_hex( from[len+ 3] ) && check_hex( from[len+ 4] ) &&
	    from[len+ 5] == ':'       &&
	    check_hex( from[len+ 6] ) && check_hex( from[len+ 7] ) &&
	    from[len+ 8] == ':'       &&
	    check_hex( from[len+ 9] ) && check_hex( from[len+10] ) &&
	    from[len+11] == ':'       &&
	    check_hex( from[len+12] ) && check_hex( from[len+13] ) &&
	    from[len+14] == ':'       &&
	    check_hex( from[len+15] ) && check_hex( from[len+16] ) &&
	    (from[len+17] == 0 || from[len+17] == ' ')
	)
	{
		...
		if( len < sz )
		{
			*boot_command_line = ' ';
			boot_command_line++;
			memcpy( boot_command_line, from, len-1 );
			boot_command_line += len - 1;
			*boot_command_line = 0;
		}
	}
...
static int __init parse_tag_mem32(const struct tag *tag)
{
	...
	galois_printk("Bootloader sets memory start 0x%08x, size %dKB\n", 
			tag->u.mem.start, tag->u.mem.size / 1024);
#endif
	return 0;
}
...
void __init setup_arch(char **cmdline_p)
{
	...
	create_command_line(boot_command_line, from, COMMAND_LINE_SIZE);
