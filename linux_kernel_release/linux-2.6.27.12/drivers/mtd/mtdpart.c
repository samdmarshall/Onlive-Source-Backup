...
int check_mtd_badblock(struct mtd_info *mtd)
{
	struct mtd_part *part = PART(mtd);
	if(!part->master)
		return 0;
	if(part->master->block_isbad) {
		uint32_t offs = 0;
		while(offs < part->mtd.size) {
			if(part->master->block_isbad(part->master, offs + part->offset))
...
EXPORT_SYMBOL(check_mtd_badblock);
#endif
