...
static const char SysKonnectFileId[] = "@(#)"
	"$Id: skxmac2.c,v 2.112 2010/03/29 14:59:29 rschmidt Exp $ (C) Marvell.";
#endif
...
int		Port)	/* Port Index (MAC_1 + n) */
{
	...
	if (pAC->GIni.GIYukonLite &&
		...
		SK_IN8(IoC, PCI_C(pAC, PSM_CONFIG_REG4), &Byte);
		...
		SK_OUT8(IoC, PCI_C(pAC, PSM_CONFIG_REG4), Byte);
...
int		Port)	/* Port Index (MAC_1 + n) */
{
	...
	if (CHIP_ID_YUKON_2(pAC)) {
		...
		Byte = (pAC->GIni.GIChipId == CHIP_ID_YUKON_FE_P) ? (SK_U8)BIT_7S : 0;
	...
	if (pAC->GIni.GP[Port].PRevAutoNeg) {

		SK_IN32(IoC, MR_ADDR(Port, GMAC_CTRL), &DWord);

		/* set PHY Reverse Auto-Negotiation */
		DWord &= ~GMC_RST_SET;
...
SK_U8	Mode)		/* low power mode */
{
	...
	if (ChipId == CHIP_ID_YUKON_EC_U ||
		...
		SK_IN16(IoC, PCI_C(pAC, PEX_CAP_REGS(PEX_LNK_CTRL)),
				...
				if (pAC->GIni.GIGotoD3Cold) {
					...
					if (HW_FEATURE(pAC, HWF_WA_DEV_4200)) {
						...
						if ((pAC->GIni.GIPexLinkCtrl & PEX_LC_CLK_PM_ENA) == 0) {
							...
							SK_OUT16(IoC, PCI_C(pAC, PEX_CAP_REGS(PEX_LNK_CTRL)),
								pAC->GIni.GIPexLinkCtrl);
						}
					}
