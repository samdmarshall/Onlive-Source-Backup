...
#ifdef MV_INCLUDE_FW_SDK_LINK_MAINTENANCE
	pAC->GIni.GIDontInitPhy = SK_FALSE;
	pAC->UseLinkMaintenance = SK_TRUE;
#endif
...
struct SK_NET_DEVICE *dev)  /* the device that is to be opened */
{
	...
	printk("SkGeOpen: pAC->Suspended:          %d\n",
		pAC->Suspended);
	printk("SkGeOpen: pAC->UseLinkMaintenance: %d\n",
		pAC->UseLinkMaintenance);
...
struct SK_NET_DEVICE *dev)  /* the device that is to be closed */
{
	...
	printk("SkGeClose: pAC->Suspended:          %d\n",
		pAC->Suspended);
	printk("SkGeClose: pAC->UseLinkMaintenance: %d\n",
		pAC->UseLinkMaintenance);
...
#define M_CurrPort pAC->GIni.GP[Port]
	...
	*/
	...
	if (LinkMaintenance[pAC->Index] != NULL) {
		if (strcmp(LinkMaintenance[pAC->Index], "On") == 0) {
			...
			pAC->UseLinkMaintenance = SK_FALSE;
		} else {
	...
	*/
	...
	/*
	** Decide whether to set new config value if something valid has
	** been received.
	*/
...
#ifdef MV_INCLUDE_SDK_SUPPORT
#ifdef MV_INCLUDE_FW_SDK_LINK_MAINTENANCE
		GM_IN16(pAC->IoBase, FromPort, GM_GP_CTRL, &GmGpCtrl);
		pAC->DriverLinkStat = GmGpCtrl;
#endif
#endif
		/* MAC update */
		SkAddrMcUpdate(pAC, IoC, FromPort);
