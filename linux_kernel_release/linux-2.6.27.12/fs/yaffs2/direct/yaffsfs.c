...
#include <fcntl.h> // for file ctrl flags
#include "yaffsfs.h"
...
int yaffs_mount(const YCHAR *path)
{
	...
	if(dev)
	{
		...
		else
		{
			...
			fprintf(stderr, "device already mounted\n");
		}
	}
	else
	{
		...
		fprintf(stderr, "can't find root %s in cfg list\n", path);
	}
	yaffsfs_Unlock();
	return retVal;
...
int yaffs_sync(const YCHAR *path)
{
        ...
        if(dev)
        {
                ...
                else
                {
                        ...
                        fprintf(stderr, "device not mounted\n");
                }
        }
        else
        {
	...
	if(dev)
	{
		if(dev->isMounted)
		{
			...
			else
			{
				...
				yaffsfs_SetError(-EBUSY);
				fprintf(stderr, "device is busy, can't be unmounted\n");
			}
