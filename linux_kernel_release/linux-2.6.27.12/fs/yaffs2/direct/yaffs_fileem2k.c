...
#define BLOCKS_PER_HANDLE (SIZE_IN_MB*BLOCKS_PER_MB)//8000
...
static yflash_Device filedisk = {.filediskpath = 0};
...
static int GetBlockFileHandle(int n)
{
	...
	if(!filedisk.filediskpath){
		...
		h =  open(filedisk.filediskpath, O_RDWR, S_IREAD | S_IWRITE);
		if(h >= 0){
			fstat(h, &statbuf);
			if(statbuf.st_size != filedisk.nBlocks*PAGES_PER_BLOCK*PAGE_SIZE){
				printf("wrong file disk size\n");
				close(h);
				abort();
				return -1;
			}
...
void SetFileDiskPath(char * path)
{
	if(path){
		filedisk.filediskpath = (char*)malloc(strlen(path)+1);
		strcpy(filedisk.filediskpath, path);
	}
}
...
void SetFileDisk(char * path, int diskSizeInMB)
{
	if(!path)
		return;
	SetFileDiskPath(path);
	filedisk.nBlocks = diskSizeInMB*BLOCKS_PER_MB;
	SIZE_IN_MB = diskSizeInMB;
}

void FileDiskDeInit()
{
	if(filedisk.filediskpath)
		free(filedisk.filediskpath);
}
...
int yflash2_WriteChunkWithTagsToNAND(yaffs_Device *dev,int chunkInNAND,const __u8 *data, const yaffs_ExtendedTags *tags)
{
	...
	if(dev->inbandTags){
		...
		if(tags)
		{
			pos = (chunkInNAND % (PAGES_PER_BLOCK * BLOCKS_PER_HANDLE)) * PAGE_SIZE + PAGE_DATA_SIZE + YAFFS2_TAG_OFFSET;
			h = filedisk.handle[(chunkInNAND / (PAGES_PER_BLOCK * BLOCKS_PER_HANDLE))];
