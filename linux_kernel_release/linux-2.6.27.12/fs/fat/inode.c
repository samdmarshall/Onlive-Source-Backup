		   ...
		   const struct inode_operations *fs_dir_inode_ops, int isvfat)
{
	...
	if (!sbi->fat_length && b->fat32_length) {
		...
		/* Must be FAT32 */
		tmp = (char *)bh->b_data;
		sbi->volume_id =le32_to_cpu(*((int *)&tmp[0x43]));/* get serial number */
		printk("get serail number %d for fat32\n",sbi->volume_id);
		sbi->fat_bits = 32;
		...
		char *tmp;
		tmp = (char *)bh->b_data;
		sbi->volume_id =le32_to_cpu(*((int *)&tmp[0x27]));/* get serial number */
		printk("get serail number %d for fat\n",sbi->volume_id);
	}
