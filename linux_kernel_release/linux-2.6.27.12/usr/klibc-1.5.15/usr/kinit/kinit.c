...
static const char *find_init(const char *root, const char *user)
{
	const char *init_paths[] = {
		"/bin/srvd", "/sbin/init", "/bin/init", "/etc/init", "/bin/sh", NULL
	};
	const char **p;
	const char *path;
...
extern void do_rootfs_OU(void);

static void make_devices()
{
	int console_stat = access("/root/dev/console", F_OK);
	int null_stat = access("/root/dev/null", F_OK);

	if (console_stat || null_stat) {
		mount("none", "/root/dev", "tmpfs", 0, NULL);
	}
	if (console_stat) {
