...
private:
    ...
    // An optional thread CPU load measurement
	cyg_uint64			time_start;			// the start point of this thread
	cyg_uint64			time_used;			// the time of this thread used
	cyg_uint64			time_period_used[8];	// the time of this thread used in this period
	cyg_uint32			cpuload_switches;	// the counter of this thread switches
	cyg_uint32			period_switches[8];	// the counter of this thread switches in this period
	cyg_uint64			time_start_swtich;	// the start point of this thread switch

public:    
	
    // function to get the data of thread CPU load measurement
	cyg_uint64 			get_cpuload_time_start() 	{ return time_start;	};
	cyg_uint64			get_cpuload_time_used() 	{ return time_used;	};
	cyg_uint64			get_cpuload_time_period_used( cyg_uint32 index ) 	{ return time_period_used[index];	};
	cyg_uint32			get_cpuload_switches() { return cpuload_switches;	};
	cyg_uint32			get_period_switches( cyg_uint32 index ) { return period_switches[index];	};
	void				reset_cpuload_time_period( cyg_uint32 index ) { time_period_used[index] = 0;	};
	void				reset_cpuload_switches_period( cyg_uint32 index ) { period_switches[index] = 0;	};
...
private:
	...
	cyg_uint32			timeslice;			// the precise value of timeslice

public:  

	cyg_uint32 			get_timeslice();

	void 				set_timeslice(cyg_ucount32 timeslice_thread);	
...
inline cyg_uint32 Cyg_Thread::get_timeslice()
{
    return timeslice;
}

inline void Cyg_Thread::set_timeslice(cyg_ucount32 timeslice_thread)
{
    timeslice = timeslice_thread;
}
