...
cyg_ucount32 Cyg_Scheduler_Implementation::system_clock_count[CYGNUM_KERNEL_CPU_MAX];
	
cyg_ucount32 Cyg_Scheduler_Implementation::timeslice_period;
...
#ifdef CYGSEM_KERNEL_SCHED_TIMESLICE_PRECISE      
	timeslice_period = GET_TIMESLICE_PERIOD();
#endif    
...
#ifdef CYGSEM_KERNEL_SCHED_TIMESLICE_PRECISE      
        timeslice_count[i] = timeslice_period;
        system_clock_count[i] = GET_SYSTEM_CLOCK();
#else        
...
Cyg_Scheduler_Implementation::schedule(void)
{
	...
	#ifdef CYGSEM_KERNEL_SCHED_MLQUEUE_ROTATION

    	// When a high priority task interrupt current running task
    	// put current task into tail of the running queue of its priority 	
        if(( current_thread[0]->get_state() == Cyg_Thread::RUNNING ) &&
        	( current_thread[0]->priority > (cyg_priority)index ))
        {
	        Cyg_Scheduler *sched = &Cyg_Scheduler::scheduler;
	        	
            CYG_ASSERTCLASS( sched, "Bad scheduler");
            
	        cyg_priority pri = current_thread[0]->priority;
...
#ifdef CYGSEM_KERNEL_SCHED_TIMESLICE_PRECISE
			// if no reschedule, reset current timeslice
			else
            	timeslice_count[cpu_this] = timeslice_period;
#else   
...
// Don't reset timeslice counter for precise time slice mode
#ifndef CYGSEM_KERNEL_SCHED_TIMESLICE_PRECISE            
