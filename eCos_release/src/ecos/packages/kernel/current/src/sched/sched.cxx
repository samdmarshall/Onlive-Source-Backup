...
#if 0
#include <../../../../../vidcode/GaloisSoftware/sys_monitor/source/mon_cpuswitch_ecos.inl>	// system monitor cpu swtich inlines
#else
#include "mon_cpuswitch_ecos.inl"	// system monitor cpu swtich inlines
#endif
#endif
...
externC long long MV_Get_System_Time_Tick( void );
    ...
    do {
        ...
        if( current->state != Cyg_Thread::RUNNING || get_need_reschedule() ) {
            ...
            if( current != next )
            {
				...
				// next thread measurement
				next->time_start_swtich = MV_Get_System_Time_Tick();
				next->cpuload_switches ++;
				for( i = 0 ; i < 8 ; i ++ )
					next->period_switches[i] ++;
				
				// current thread measurement
				temp = next->time_start_swtich - current->time_start_swtich;
				current->time_used += temp;
				for( i = 0 ; i < 8 ; i ++ )
					current->time_period_used[i] += temp;
#endif

#ifdef CYGFUN_KERNEL_THREADS_CPUSWITCH

				MV_TEST_CPU_SWITCH_TASK_Add(thread_switches[CYG_KERNEL_CPU_THIS()],
											current->unique_id,
											current->state,
											next->unique_id,
											next->state);
...
#ifdef CYGSEM_KERNEL_SCHED_TIMESLICE_PRECISE

				cyg_ucount32 current_system_clock = GET_SYSTEM_CLOCK();
				cyg_ucount32 delta_clock = DELTA_SYSTEM_CLOCK(current_system_clock, system_clock_count[CYG_KERNEL_CPU_THIS()]);

#ifdef CYGFUN_KERNEL_THREADS_CPUSWITCH
				MV_TEST_CPU_SWITCH_CLOCK_Add(current_system_clock, system_clock_count[CYG_KERNEL_CPU_THIS()]);
#endif
				// update current time slice
				if( timeslice_count[CYG_KERNEL_CPU_THIS()] <= delta_clock )
					timeslice_count[CYG_KERNEL_CPU_THIS()] = 0;
				else
					timeslice_count[CYG_KERNEL_CPU_THIS()] -= delta_clock;

				current->set_timeslice(timeslice_count[CYG_KERNEL_CPU_THIS()]);
				
#ifdef CYGFUN_KERNEL_THREADS_CPUSWITCH
				cyg_ucount32 last_timeslice_count = timeslice_count[CYG_KERNEL_CPU_THIS()];
#endif
				// update system clock count
				system_clock_count[CYG_KERNEL_CPU_THIS()] = current_system_clock;
				
				// Restore next thread timeslice
				// if next thread time slice is zero, reset it
				timeslice_count[CYG_KERNEL_CPU_THIS()] = next->get_timeslice();
				if(( timeslice_count[CYG_KERNEL_CPU_THIS()] == 0 ) || (timeslice_count[CYG_KERNEL_CPU_THIS()] > timeslice_period))
					 reset_timeslice_count();

#ifdef CYGFUN_KERNEL_THREADS_CPUSWITCH
				MV_TEST_CPU_SWITCH_TIMESLICE_Add(last_timeslice_count, timeslice_count[CYG_KERNEL_CPU_THIS()]);
#endif
#endif
                // Switch contexts
			...
			// We return here only when the current thread is rescheduled
			// so there is no reset timeslice
...
#ifdef CYGSEM_KERNEL_SCHED_TIMESLICE
#ifdef CYGSEM_KERNEL_SCHED_TIMESLICE_PRECISE
	// Don't reset the timeslice, bucause it will be set by unlock_inner
	// the timeslice in thread is initialized by 0;
#else
    // Reset the timeslice counter so that this thread gets a full
    // quantum. 
    reset_timeslice_count();
...
void Cyg_Scheduler::start()
{
	...
	#ifdef CYGFUN_KERNEL_THREADS_CPUSWITCH_INITSTATUS
		flag_cpu_switch = 1;
	#else
		flag_cpu_switch = 0;
	#endif
	
	MV_CC_ICCFIFOInit(pCPUSwtichList, TEST_CPU_SWITCH_LIST_ELEMENT_NUMBER, TEST_CPU_SWITCH_LIST_ELEMENT_SIZE);
