                                ...
                                cyg_thread_info *info ) __THROW
{
    ...
    if( thread->get_unique_id() == id && info != NULL )
    {
...
#ifdef CYGFUN_KERNEL_THREADS_CPULOAD
		{
			...
			info->cpuload = 1;
			info->time_start = thread->get_cpuload_time_start();
			info->time_used = thread->get_cpuload_time_used();
			info->cpuload_switches = thread->get_cpuload_switches();
			for( i = 0 ; i < 8 ; i ++ )
			{
				info->time_period_used[i] = thread->get_cpuload_time_period_used( i );
				info->period_switches[i] = thread->get_period_switches( i );
			}
		}
#else
		info->cpuload = 0;
...
cyg_bool_t cyg_thread_reset_cpuload( cyg_handle_t threadh, cyg_uint32 index ) __THROW
{
    cyg_bool_t result = true;
#ifdef CYGFUN_KERNEL_THREADS_CPULOAD    
    Cyg_Thread *thread = (Cyg_Thread *)threadh;
...
#ifdef CYGFUN_KERNEL_THREADS_CPULOAD
	thread->reset_cpuload_time_period( index );
	thread->reset_cpuload_switches_period( index );
#else
	result = false;
#endif   
