...
static const struct dhcp_client_t ext_dhcp_clients[] = {
	...
	{ "udhcpc",
		"udhcpc -T 5 -R -b -V 'OnLive 1.0' -H 'MicroConsole' -p /var/run/udhcpc.%iface%.pid -i %iface%[[ -H %hostname%]][[ -c %clientid%]]"
...
static int dhcp_up(struct interface_defn_t *ifd, execfn *exec)
{
	...
	if (!execute("ifconfig %iface%[[ hw %hwaddress%]] up", ifd, exec))
		return 0;
#endif
	return execute("udhcpc -T 5 -R -b -V 'OnLive 1.0' -H 'MicroConsole' -p /var/run/udhcpc.%iface%.pid "
