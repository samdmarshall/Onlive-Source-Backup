...
static int send_discover(uint32_t xid, uint32_t requested)
{
	...
	packet.xid = xid;
	// Hardcode secs field to 5 to overcome issue where some DHCP servers
	// wait a given timeout before responding
	// This is necessary because the secs field in not incremented automatically
	// as it should be
	// Example: Mac OSX 10.6.5 running internet connection sharing has a timeout value of 4
	packet.secs = htons(5);
	if (requested)
