...
static int setCustomBaud(int speed)
{
	int val;
	void *pdata;
	int mem_length = 0x200000;
	int fd;
	...
	pdata = mmap(0 , mem_length , PROT_READ|PROT_WRITE, MAP_SHARED, fd , 0xf7E00000);
	if ((int)pdata==-1)
	{
		perror("mmap failed");
		close(fd);
	...
	val = __raw_readl ( pdata + 0x1CA00C );
	//printf("0x1CA00C=%x\n",val);
	val |= 0x00000080;
	__raw_writel(val,pdata + 0x1CA00C);
	//val = __raw_readl ( pdata + 0x1CA00C );
	//printf("0x1CA00C=%x\n",val);
	//val = __raw_readl ( pdata + 0x1CA000 );
	//printf("0x1CA000=%x\n",val);
	if (speed == 312500)
		__raw_writel(4,pdata + 0x1CA000);
	else if (speed == 625000)
		__raw_writel(2,pdata + 0x1CA000);
	else 
	{
	...
	//val = __raw_readl ( pdata + 0x1CA000 );
	//printf("0x1CA000=%x\n",val);
	//val = __raw_readl ( pdata + 0x1CA004 );
	//printf("0x1CA004=%x\n",val);
...
static int ol_bcsp(int fd, struct uart_t *u, struct termios *ti)
{
	unsigned char byte, bcsph[4], bcspp[4],
		bcsp_sync_resp_pkt[10] = {0xc0,0x00,0x41,0x00,0xbe,0xac,0xaf,0xef,0xee,0xc0},
		bcsp_conf_resp_pkt[10] = {0xc0,0x00,0x41,0x00,0xbe,0xde,0xad,0xd0,0xd0,0xc0},
	...
	if ((u->speed == 312500) || (u->speed == 625000))
	{
		setCustomBaud(u->speed);
	}
...
struct uart_t uart[] = {
	...
	/* OnLive MC FCS BCSP */
	{ "ol_bcsp",    0x0, 0x0, HCI_UART_BCSP,   115200, 312500, 0, NULL, ol_bcsp  },
